...
{{-- ⬇️ Teleport the panel to <body> using Alpine --}}
<template x-teleport="body">
  <div
    ...
    class="{{ $panel }}"
    :aria-activedescendant="activeOptionId() || null"
    @keydown.escape.prevent.stop="close(); $nextTick(() => requestAnimationFrame(() => $refs.trigger?.focus()))"
    @mousedown.prevent

    {{-- Floating UI init (unchanged, apart from using window.FloatingUIDOM below) --}}
    x-init="
      (async () => {
            ...
            middleware: [
              ...
              size({
                apply({ availableHeight, elements }) {
                  const cap = Math.max(160, Math.floor(availableHeight));
                  elements.floating.style.setProperty('--combobox-max-h', cap + 'px');
                },
              }),
            ],
          });
        ...
        $watch('open', (o) => {
          if (o) {
            $nextTick(() => {
              cleanup = autoUpdate(reference, floating, update, { elementResize: true, ancestorScroll: true, ancestorResize: true });
              update();
            });
          } else {
    ...
    <div class="border-b" x-show="searchable">
      ...
      <div class="relative">
        <input
          ...
          aria-label="{{ __('Search options', 'wordpress-quickstart') }}"
          ...
          @mousedown.stop
        />
        <x-lucide-search aria-hidden="true" class="text-muted-foreground pointer-events-none absolute top-1/2 left-2.5 size-4 -translate-y-1/2" />
      </div>
    </div>

    <div
      class="max-h-[var(--combobox-max-h,320px)] overflow-auto p-1"
      style="--combobox-max-h: {{ $maxHeight }}"
    >
      {{ $slot }}
