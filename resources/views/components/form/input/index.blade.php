...
@php
  ...
  $baseInputClass = 'file:text-foreground placeholder:text-muted-foreground 
        ...
        border-input flex h-9 min-w-0 rounded-md border bg-background px-3 py-1 
  ...
  // Base label class
  $baseLabelClass = 'mt-2 flex items-center text-sm leading-none font-medium select-none';
  $wooLabelClass = 'group-[.woocommerce-invalid]:text-destructive';

  // Merge all label classes
  $mergedLabelClass = $tw->merge($iswoocommerce ? $wooLabelClass : '', $baseLabelClass, $hidelabel ? 'sr-only' : '', $labelclass);
@endphp
...
>
  @if ($label)
    <label for="{{ $id }}" class="{{ $mergedLabelClass }}">
      {{ $label }}
    </label>
  @endif
